    ...
    IHubContext<PresenceHub> presenceHub) : Hub
{
    ...
    public async Task SendMessage(CreateMessageDto createMessageDto)
    {
        ...
        if (username == createMessageDto.RecipientUsername.ToLower()) throw new HubException("Cannot sent message to yourself");
        var sender = await userRepository.GetUserByUsernameAsync(username);
        var recipient = await userRepository.GetUserByUsernameAsync(createMessageDto.RecipientUsername);
        if (sender == null || recipient == null) throw new HubException("Cannot send message at this time");
        if (sender.UserName is null || recipient.UserName is null) throw new HubException("Sender ot recipient username cannot be null");
